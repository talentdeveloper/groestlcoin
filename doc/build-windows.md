...
and is the platform used to build the Groestlcoin Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Groestlcoin Core applications.
...
causes two of the groestlcoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the groestlcoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the groestlcoin source code).
